...
def build_official_source_queries() -> dict[str, str]:
    ...
    location_join_for_payroll = f"""LEFT JOIN `{TABLES['location_catalog']}` AS location_catalog
...
WHERE {common_month_window}
  ...
  AND source.Division_Nombre = @division_name
...
def load_source_results_from_bigquery(reporting_year: int, last_calendar_month: int) -> dict[str, list[dict[str, Any]]]:
    """Ejecuta las cuatro consultas oficiales con parámetros tipados y auditables."""
    ...
    query_parameters = [
        ...
        bigquery.ScalarQueryParameter("division_name", "STRING", SCOPE["division"]),
