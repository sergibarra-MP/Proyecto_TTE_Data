"""Punto de entrada: carga, procesa y genera las tres vistas del dashboard."""
...
from homologated_tte_enrichment_loader import (
    load_homologated_tte_enriched_results_from_bigquery,
    load_homologated_tte_enriched_results_from_json,
)
from homologated_tte_enrichment_processors import (
    compile_homologated_tte_enriched_cube,
    compile_homologated_tte_enriched_reason_series,
)
...
def parse_command_line_arguments() -> argparse.Namespace:
    """Define argumentos reproducibles y alternativas JSON para las tres vistas."""

    current_date = date.today()
    parser = argparse.ArgumentParser(description="Genera las vistas Homologada, Team TTE y Homologada+TTE.")
    ...
    parser.add_argument("--homologated-tte-source-results-json", type=Path)
    parser.add_argument(
        "--output-html",
        type=Path,
...
def main() -> None:
    """Ejecuta fuentes separadas y sólo combina sus contratos en el HTML final."""
    ...
    if arguments.team_tte_source_results_json:
        ...
        # Una prueba local homologada no debe conectarse silenciosamente a BigQuery.
    ...
    if arguments.homologated_tte_source_results_json:
        enriched_source_results = load_homologated_tte_enriched_results_from_json(arguments.homologated_tte_source_results_json)
    elif arguments.source_results_json:
        # El modo fixture permanece totalmente local y explícito.
        enriched_source_results = {}
    else:
        enriched_source_results = load_homologated_tte_enriched_results_from_bigquery(arguments.year, arguments.last_month)
    ...
    enriched_cube_cells = compile_homologated_tte_enriched_cube(enriched_source_results)
    enriched_reason_cells = compile_homologated_tte_enriched_reason_series(enriched_source_results)

    write_dashboard_html(
        monthly_cube_cells=monthly_cube_cells,
        reason_cells=reason_cells,
        team_tte_payload=team_tte_payload,
        homologated_tte_enriched_cube_cells=enriched_cube_cells,
        homologated_tte_enriched_reason_cells=enriched_reason_cells,
        output_html_path=arguments.output_html,
        reporting_year=arguments.year,
        last_calendar_month=arguments.last_month,
    )
    ...
    print(f"Vista Homologada+TTE: {len(enriched_cube_cells):,} celdas mensuales y {len(enriched_reason_cells):,} celdas de motivos.")
