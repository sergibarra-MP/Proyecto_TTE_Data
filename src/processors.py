...
from data_loader import BUSINESS_RULES, SCOPE
...
def build_cube_cell_key(source_row: dict[str, Any], segment: str) -> tuple[Any, ...]:
    ...
    return (int(source_row["year"]), int(source_row["month"]), segment, str(source_row.get("region") or "Sin especificar"), str(source_row.get("operation_type") or "Sin especificar"), str(source_row.get("site_operativo") or "Sin especificar"), str(source_row.get("area") or "Sin especificar"), str(source_row.get("subarea") or "Sin especificar"))
...
def create_empty_cube_cell(dimension_key: tuple[Any, ...]) -> dict[str, Any]:
    ...
    year, month, segment, region, operation_type, site_operativo, area, subarea = dimension_key
    return {"year": year, "month": month, "segment": segment, "region": region, "operationType": operation_type, "siteOperativo": site_operativo, "area": area, "subarea": subarea, **{metric_name: 0.0 for metric_name in NUMERIC_METRIC_NAMES}}
...
def compile_monthly_dashboard_cube(source_results_by_query_name: dict[str, list[dict[str, Any]]]) -> list[dict[str, Any]]:
    ...
    return sorted(cells_by_key.values(), key=lambda cell: (cell["year"], cell["month"], cell["segment"], cell["region"], cell["operationType"], cell["siteOperativo"], cell["area"], cell["subarea"]))
